      ...
      <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden bg-background border-border rounded-2xl">
        <div className="bg-secondary/40 p-6 border-b border-border">
          <div className="flex justify-between items-start">
            <div>
              <DialogTitle className="text-xl font-bold">{user.full_name || user.username || "Anonymous"}</DialogTitle>
              <DialogDescription className="mt-1 font-mono text-xs text-muted-foreground">{user.id}</DialogDescription>
            </div>
            <div className="flex gap-1 flex-wrap justify-end items-center">
              {user.roles?.map((r: string) => (
                <span key={r} className={cn("px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider",
        ...
        <div className="flex border-b border-border bg-card px-4">
          <button onClick={() => setTab("overview")} className={cn("px-4 py-3 text-sm font-semibold border-b-2 transition-colors", tab === "overview" ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground")}>Overview</button>
          <button onClick={() => setTab("wallet")} className={cn("px-4 py-3 text-sm font-semibold border-b-2 transition-colors", tab === "wallet" ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground")}>Manage Wallet</button>
          <button onClick={() => setTab("orders")} className={cn("px-4 py-3 text-sm font-semibold border-b-2 transition-colors", tab === "orders" ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground")}>Orders</button>
        </div>
          ...
          {tab === "overview" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              {user.roles?.includes("agent") && !user.agentProfile?.activation_paid && (
                <div className="p-5 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 animate-in fade-in slide-in-from-top-2">
                  ...
                  <Button 
                    ...
                    className="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-xl shrink-0"
                  >
                    Verify & Activate
                  </Button>
                </div>
              ...
              <div className="flex items-center justify-between p-5 rounded-2xl bg-primary/5 border border-primary/10">
                ...
                <Button variant="outline" size="sm" onClick={() => setTab("wallet")} className="rounded-xl font-semibold bg-background">Adjust</Button>
              </div>

              <div>
                <h3 className="font-semibold text-sm text-muted-foreground uppercase tracking-wider mb-3">Recent Transactions</h3>
                {loadingTx ? (
                  <div className="py-8 flex justify-center"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>
                ) : transactions?.length === 0 ? (
                  <div className="py-8 text-center text-sm text-muted-foreground border border-dashed border-border rounded-2xl">No transactions found</div>
                ) : (
                  <div className="space-y-2">
                    {transactions?.slice(0, 5).map(tx => (
                      <div key={tx.id} className="flex justify-between items-center p-3 rounded-xl border border-border bg-card">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-semibold capitalize">{(tx.type || 'unknown').replace("_", " ")}</p>
                        ...
                        <div className="text-right">
                          <p className={cn("text-sm font-semibold", tx.amount > 0 ? "text-emerald-500" : "text-foreground")}>{tx.amount > 0 ? "+" : ""}{formatGHS(tx.amount)}</p>
                          <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">{tx.status}</p>
                        </div>
                      </div>
                    ))}
                    {transactions && transactions.length > 5 && (
                       <div className="text-center pt-2 text-xs font-semibold text-muted-foreground">Showing 5 of {transactions.length}</div>
                    )}
                  </div>
                )}
              </div>
          ...
          {tab === "wallet" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="p-5 rounded-2xl border border-border bg-card space-y-4">
                <h3 className="font-semibold text-lg flex items-center gap-2"><Wallet className="h-5 w-5 text-primary" /> Adjust Wallet Balance</h3>
                ...
                <div className="space-y-3 pt-2">
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5 block">Amount (GHS)</label>
                    <Input type="number" value={adjustAmount} onChange={e => setAdjustAmount(e.target.value)} placeholder="0.00" className="h-11 rounded-xl text-lg font-semibold" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5 block">Reason (Visible to user)</label>
                  ...
                  <div className="flex gap-3 pt-2">
                    <Button onClick={() => handleWalletAdjust("credit")} disabled={adjusting} className="flex-1 h-11 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-semibold">
                      {adjusting ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <ArrowDownLeft className="h-4 w-4 mr-2" />} Credit Wallet
                    </Button>
                    <Button onClick={() => handleWalletAdjust("debit")} disabled={adjusting} variant="outline" className="flex-1 h-11 rounded-xl border-rose-500/30 text-rose-500 hover:bg-rose-500/10 font-semibold">
              ...
              <div className="p-5 rounded-2xl border border-rose-500/20 bg-rose-500/5 space-y-4">
                <h3 className="font-semibold text-lg text-rose-500 flex items-center gap-2"><KeyRound className="h-5 w-5" /> Danger Zone</h3>
                <div className="space-y-3">
                  <div>
                    <label className="text-xs font-semibold text-rose-500/70 uppercase tracking-wider mb-1.5 block">Force Reset Password</label>
                    <div className="flex gap-2">
                      <Input value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="New strong password" type="text" className="h-11 rounded-xl border-rose-500/30 bg-background" />
                      <Button onClick={handlePasswordReset} disabled={resetting} className="h-11 rounded-xl bg-rose-500 hover:bg-rose-600 text-white font-semibold shrink-0">
          ...
          {tab === "orders" && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
               {loadingOrders ? (
                  ...
                  <div className="py-8 text-center text-sm text-muted-foreground border border-dashed border-border rounded-2xl">No orders found</div>
                ) : (
                  <div className="space-y-3">
                    {orders?.map(o => (
                      <div key={o.id} className="p-4 rounded-xl border border-border bg-card flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                         <div>
                           <p className="font-semibold">{o.network?.name} {o.bundle?.size_label}</p>
