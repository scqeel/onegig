  ...
  return (
    <div className="min-h-screen bg-[#050811] text-slate-100 flex flex-col justify-between overflow-x-hidden selection:bg-purple-500 selection:text-white">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute -top-32 left-1/2 -translate-x-1/2 w-[800px] h-[800px] bg-purple-600/15 rounded-full blur-[140px] animate-pulse-subtle" />
        <div className="absolute top-1/3 -right-40 w-[500px] h-[500px] bg-indigo-600/15 rounded-full blur-[120px]" />
        <div className="absolute bottom-10 -left-40 w-[500px] h-[500px] bg-fuchsia-600/15 rounded-full blur-[120px]" />
        <div className="absolute inset-0 grid-pattern-dark opacity-40" />
      </div>

      {/* Floating Pill Header Navbar */}
      <header className="fixed left-0 right-0 top-4 z-50 px-4 md:px-8 pointer-events-none">
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between rounded-full border border-white/10 bg-slate-950/80 px-4 md:px-6 shadow-[0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-xl pointer-events-auto transition-all">
          <Logo size="md" />

          <nav className="flex items-center gap-2">
            <Link
              to="/track"
              className="inline-flex h-9 items-center gap-1.5 rounded-full px-4 text-xs font-bold text-slate-300 hover:bg-white/10 transition-colors"
            >
              <Search className="h-3.5 w-3.5" /> Track Order
            </Link>
            <Button
              asChild
              className="h-9 rounded-full px-5 text-xs font-extrabold bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-lg hover:shadow-purple-500/20"
            >
              <Link to="/buy">
      ...
      <main className="relative z-10 pt-28 pb-16 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto w-full">
        ...
        <div className="text-center space-y-4 mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight leading-tight">
            ...
            <span className="hero-gradient-text-v2">& Beneficiary Eligibility</span>
          </h1>

          <p className="max-w-xl mx-auto text-sm sm:text-base text-slate-400 leading-relaxed font-medium">
        ...
        <div className="glass-card-grace rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl relative">
          <form onSubmit={handleVerify} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                <Smartphone className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-purple-400 transition-colors" />
                <Input
                  type="tel"
                  inputMode="numeric"
                  value={phone}
                  ...
                  className="h-14 rounded-2xl border-slate-800 bg-[#070b19] pl-12 pr-4 text-base font-bold text-white placeholder:text-slate-600 focus:border-purple-500/60 focus:ring-2 focus:ring-purple-500/20"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  { code: "MTN", name: "MTN Ghana", color: "from-amber-500/20 to-yellow-500/20 border-amber-500/40 text-amber-300" },
                  { code: "TELECEL", name: "Telecel", color: "from-red-500/20 to-rose-500/20 border-red-500/40 text-red-300" },
                  { code: "AIRTELTIGO", name: "AirtelTigo (AT)", color: "from-blue-500/20 to-purple-500/20 border-blue-500/40 text-blue-300" },
                ].map((net) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex flex-col items-center justify-center p-3.5 rounded-2xl border transition-all text-xs font-bold ${
                        active
                          ? `bg-gradient-to-b ${net.color} shadow-lg ring-2 ring-purple-500/30 scale-[1.02]`
                          : "border-slate-800 bg-slate-950/40 text-slate-400 hover:border-slate-700 hover:text-slate-200"
                      }`}
                    >
                      {active && (
                        <span className="absolute top-2 right-2 flex h-4 w-4 items-center justify-center rounded-full bg-purple-500 text-white">
            ...
            <div className="flex items-center justify-between rounded-2xl border border-white/5 bg-slate-950/40 p-4">
              <div className="space-y-0.5">
                <Label htmlFor="ported-switch" className="text-xs font-extrabold text-slate-200">
                  Ported Phone Number Check
                </Label>
                <p className="text-[11px] text-slate-400">Enable if this number was ported from another network (MNP).</p>
              </div>
              <Switch
                id="ported-switch"
                checked={isPorted}
            ...
            <Button
              ...
              className="h-14 w-full rounded-2xl text-sm font-black uppercase tracking-wider bg-gradient-to-r from-purple-600 via-fuchsia-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white shadow-xl shadow-purple-500/20 transition-all duration-300 hover:scale-[1.01]"
            >
              {loading ? (
                ...
                <span className="flex items-center gap-2">
                  <Sparkles className="h-4 w-4 fill-current" /> Run Live Verification Check
                </span>
              )}
            </Button>
          </form>
          ...
          {result && (
            <div className="mt-8 rounded-2xl border border-emerald-500/30 bg-slate-950/90 p-6 space-y-5 animate-in fade-in zoom-in duration-300 shadow-2xl">
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <div className="flex items-center gap-3">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${
                    result.beneficiaryVerified ? "bg-emerald-500/20 text-emerald-400" : "bg-amber-500/20 text-amber-400"
                  }`}>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Checked at {result.verifiedAt}</p>
                  </div>
                </div>

                <span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider ${
                  result.beneficiaryVerified ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/40" : "bg-amber-500/20 text-amber-400 border border-amber-500/40"
                }`}>
              ...
              <div className="grid gap-3 sm:grid-cols-2 text-xs">
                <div className="rounded-xl bg-white/5 p-3.5 space-y-1">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Subscriber Account Name</span>
                  <p className="font-extrabold text-sm text-emerald-400">{result.accountName || "Registered Telecom Subscriber"}</p>
                </div>
                <div className="rounded-xl bg-white/5 p-3.5 space-y-1">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Network & Ported Status</span>
                  <p className="font-extrabold text-sm text-slate-200">{result.networkCode} {result.isPorted ? "(Ported Number Verified)" : "(Standard)"}</p>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-white/10 space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  {filteredBundles.slice(0, 3).map((b) => (
                    <button
                      ...
                      className="group flex flex-col justify-between rounded-xl border border-purple-500/30 bg-purple-500/10 p-3.5 text-left transition-all hover:border-purple-500 hover:bg-purple-500/20"
                    >
                      <span className="text-lg font-black text-white">{b.size_label}</span>
                      <div className="mt-2 flex items-center justify-between text-xs">
                        <span className="font-bold text-purple-300">{formatGHS(Number(b.user_price ?? b.base_price))}</span>
                        <span className="text-[10px] font-bold text-purple-400 group-hover:translate-x-0.5 transition-transform flex items-center gap-0.5">
                ...
                <Button
                  ...
                  className="w-full h-12 rounded-xl text-xs font-extrabold uppercase bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black shadow-lg shadow-emerald-500/20"
                >
                  <Zap className="mr-1.5 h-4 w-4 fill-current" /> Proceed To Checkout With {result.phone}
                </Button>
              </div>
            </div>
          )}
      ...
      {/* Footer */}
      <footer className="relative z-10 border-t border-white/5 bg-slate-950/60 py-8 text-center text-xs text-slate-500">
        <div className="mx-auto max-w-5xl px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 font-semibold text-slate-400">
            ...
            <Link to="/buy" className="hover:text-white transition-colors">Buy Data</Link>
